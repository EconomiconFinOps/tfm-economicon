...
def test_successful_request_increments_http_requests_counter():
    ...
    assert 'processor_http_requests_total{method="GET",path="/ping",status_code="200"}' in body
...
def test_request_duration_uses_seconds_for_prometheus_buckets():
    ...
    assert (
        'processor_http_request_duration_seconds_bucket{le="0.25",method="GET",path="/timed"} 1.0'
        in body
    )
    assert (
        'processor_http_request_duration_seconds_bucket{le="0.1",method="GET",path="/timed"} 0.0'
        in body
    )
...
def test_unhandled_exception_is_recorded_as_500():
    ...
    body = _call(app, "GET", "/metrics").text
    assert (
        'processor_http_requests_total{method="GET",path="/boom",status_code="500"}' in body
    )
...
def test_unmatched_paths_share_a_bounded_label():
    ...
    body = _call(app, "GET", "/metrics").text
    assert (
        'processor_http_requests_total{method="GET",path="__unmatched__",status_code="404"}'
        in body
    )
