...
@pytest.mark.parametrize("key", ["replace-me-auth-secret", "short", " " * 40, "x" * 31])
def test_jwt_rejects_short_blank_and_known_placeholders(production_env, monkeypatch, key):
...
def test_demo_opt_in_requires_external_nonlegacy_password(production_env, monkeypatch, password):
