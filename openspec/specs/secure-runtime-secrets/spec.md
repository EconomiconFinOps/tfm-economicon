# secure-runtime-secrets Specification

## Purpose
Define external runtime credential configuration, safe startup and diagnostics,
secret-free build inputs and explicit credential preservation and rotation boundaries.
...
The local exception and full pre-code gate are APPROVED as recorded in the [JUP-053 proposal](../../changes/archive/2026-09-25-jup-053-secure-runtime-secrets/proposal.md). The exception SHALL require development/test, explicit opt-in defaulting to false, an allowed local target and operator-confirmed disposable isolation; production/shared use MUST remain prohibited.
