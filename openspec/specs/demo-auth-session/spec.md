...
Define the demo authentication and session contract for the MVP: login, token issuance/validation, `/me` profile resolution, frontend session lifecycle, backend CORS for browser access, and the boundary with runtime secrets. Promoted from `jup-085-auth-session-contract`.
...
- **THEN** the server profile is exposed through the protected context, the stored profile is updated, and normal tenant selection resumes.
...
- **THEN** the current identity is reconciled, selection/cache tied to the previous identity including concurrent tenant results is discarded, and tenant bootstrap repeats before exposing protected data.
...
- **THEN** the runtime guard treats it as a revalidation error and follows the strict logout path, clearing local session, tenant and cache and requesting login.
...
- **THEN** the cleanup/isolation clears existing tenant effects and ignores subsequent results and callbacks, leaving session, tenant and cache empty.
...
- **AND** validation rejects an invalid login response without creating a session or starting either bootstrap request; credential rejection remains a login error.
...
The backend SHALL use FastAPI/Starlette CORSMiddleware with `CORS_ALLOWED_ORIGINS`
parsed as a JSON list through Settings. Entries SHALL be exact serialized
HTTP(S) origins without paths, credentials, query, fragment or wildcard;
production SHALL require HTTPS, with HTTP allowed only in development/test.
No regex, arbitrary reflection or `null` origin is permitted. CORS SHALL NOT
replace JWT authentication or tenant authorization.
...
- **AND** the local examples are `http://localhost:5173` and `http://127.0.0.1:5173` only for development/test; production origins are supplied by the operator.
...
- **AND** a `500` generated outside the CORS layer by ServerErrorMiddleware is an explicit limit, not guaranteed browser-readable. Native preflight terminates before inner metrics/access logging.

#### Scenario: Canonical browser flow preserves JUP-097

- **WHEN** QA exercises real login, `/me`, `/tenants` and logout on the canonical separate-port stack with matching explicit origins, normal browser security and no proxy
...
- **AND** native frontend tests are reused; mocks or ASGI checks alone do not establish browser success.
